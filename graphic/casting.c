...
/*   Updated: 2023/08/23 20:01:05 by eelhafia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double 	dictance_virtical(t_cub *y)
{
	...
	y_fisrt_interce_v = y->plr->y_p
		+ ((x_fisrt_interce_v - y->plr->x_p) * tan(y->angle_of_ray));
	while (x_fisrt_interce_v >= 0 && y_fisrt_interce_v >= 0
		&& !check_wall_fram(x_fisrt_interce_v, y_fisrt_interce_v, y->map, y))
	{
		y_fisrt_interce_v += ystep_v;
		x_fisrt_interce_v += xstep_v;
	}
	return (
		y->hitwall_x_v = x_fisrt_interce_v, y->hitwall_y_v = y_fisrt_interce_v,
		distance_p(y->plr->x_p, y->plr->y_p,
			x_fisrt_interce_v, y_fisrt_interce_v));
}
